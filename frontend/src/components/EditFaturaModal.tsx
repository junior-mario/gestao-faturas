...
interface Props {
  fatura: Fatura
  onClose: () => void
  onSaved: (nomeFatura: string) => Promise<void> | void
}
...
export default function EditFaturaModal({ fatura, onClose, onSaved }: Props) {
  const [nome, setNome] = useState(fatura.nome)
  const [conta, setConta] = useState(fatura.conta || '-')
  const [valor, setValor] = useState(fatura.valor)
  const [moeda, setMoeda] = useState(fatura.moeda || 'BRL')
  const [valorNum, setValorNum] = useState(String(fatura.valor_num).replace('.', ','))
  const [dia, setDia] = useState(String(fatura.dia))
  const [grupo, setGrupo] = useState(fatura.grupo)
  ...
  async function handleSave() {
    ...
    try {
      ...
      await api.updateFatura(fatura.id, {
      ...
      await onSaved(nomeNormalizado)
    } catch (err) {
      console.error(err)
      setError('Nao foi possivel salvar as alteracoes da fatura.')
    } finally {
      setSaving(false)
    }
  }
  ...
  return (
    <div className="modal-overlay" onClick={handleOverlayClick}>
      <div className="modal" style={{ maxWidth: '620px' }}>
        <div className="modal-title">Editar Fatura</div>
        ...
        <div className="modal-footer">
          ...
          <button className="btn confirm" type="button" onClick={handleSave} disabled={saving}>
